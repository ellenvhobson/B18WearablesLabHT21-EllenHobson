...
data_pathname='/Users/ellenhobson/Desktop/DATA/B18_GAIT_data';
...
options.acc_threshold = 0.15*G;  % set the moving average acceleration threshold value to 15% of gravity
options.ssd_threshold = 0.3*G;  % set the moving average accel threshold to 30% of gravity
...
%-------------------------------------------------------------------------%
options.MinPeakDistance = 0.5;    % 0.5s
options.MinPeakHeight = 0.1*G;      % set to 10% of gravity [m/s^2]
%% Run Walking Detection and Step Detection Algorithm
%choose one of the filenames from the dataset and corresponding subject
%identifier
file_index = 5; %choose a value [1, 2, ..., N], where N are the number of files
